...
func _physics_process(_delta: float) -> void:
	if !sprite_node_pos_tween or not sprite_node_pos_tween.is_running():
		...
		if is_ice_next(dir, pos):
			print("is ice next")
			var ice_length = get_ice_length(dir, pos)
			var speed = 0.2 * (ice_length + 1) # change 0.2 to what speed you want
			dir = dir * (ice_length + 1) # this is hacky i know
			_move(dir, speed)
		else:
			if dir != Vector2(0, 0):
				_move(dir)
...
func get_ice_length(dir: Vector2, currpos: Vector2):
	var i: float = currpos.x
	var j: float = currpos.y
	var ice_length: int = 0
	print(i, j)
	while i >= 0 and j >= 0:
		i = i + dir.x
		j = j + dir.y
		var cell_type = tml.get_cell_atlas_coords(Vector2(i, j))
...
func _move(dir: Vector2, speed: float = 0.2):
	...
	# Istantly move the whole body to the new position
	global_position += walk_vector
	# Move the sprite back to where it was 
	$AnimatedSprite2D.global_position -= walk_vector

	if sprite_node_pos_tween:
		sprite_node_pos_tween.kill()

	# Create a new tween to move the sprite to the new position 
